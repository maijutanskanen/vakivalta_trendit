...
pxweb_query_list <-
  list("Vuosi" = c("2009", "2010", "2011", "2012", "2013", "2014",
                   ...
                   "2021", "2022", "2023", "2024"),
       "Rikos" = c("20Y", "203", "212"),
       "Uhrin sukupuoli" = c("SSS"),
       "Uhrin ikä" = c("15-17", "18-20", "21-29"),
       "Hyvinvointialue" = c("SSS"),
       "Tiedot" = c("uhri_tapkerta", "uhri_nimike", "uhri_tork"))
...
data <- data %>%
  rename(tapauskerrat = `Uhrien lukumäärä tapauskertojen mukaan (lkm.)`,
         nimiklkm = `Uhrien lukumäärä rikosnimikkeiden mukaan (lkm.)`,
         torkein = `Uhrien lukumäärä vakavimman rikoksen mukaan (lkm.)`,
         ika = `Uhrin ikä`,
         rikryhm = `Rikos`,
         vuosi = Vuosi)
...
data <- data %>%
  mutate(rikryhm = ifelse(rikryhm == "12 Henkirikokset  21:1-3 yhteensä", "hr", rikryhm),
         rikryhm = ifelse(rikryhm == "13 Tapon, murhan tai surman yritys 21:1-3,34a:1", "hry", rikryhm),
         rikryhm = ifelse(rikryhm == "1402 Törkeä pahoinpitely 21:6", "tpp", rikryhm))
